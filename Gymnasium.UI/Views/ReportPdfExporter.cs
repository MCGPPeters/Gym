        ...
        Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Content().Column(col =>
                {
                    ...
                    col.Item().Text("Configuration").FontSize(16).Bold().PaddingTop(10);
                    col.Item().Text($"Environment: {environment}\nAgent: {agent}\nEpisodes: {episodes}\nSteps per Episode: {stepsPerEpisode}").FontSize(12);
                    col.Item().Text("Summary Statistics (last 100)").FontSize(16).Bold().PaddingTop(10);
                    ...
                    col.Item().Text("Reward Curve").FontSize(16).Bold().PaddingTop(10);
                    col.Item().Image(SvgToPng(rewardChartSvg)).FitWidth();
                    col.Item().Text("Episode Length Curve").FontSize(16).Bold().PaddingTop(10);
                    col.Item().Image(SvgToPng(lengthChartSvg)).FitWidth();
                    if (!string.IsNullOrWhiteSpace(lossChartSvg))
                    {
                        col.Item().Text("Loss Curve").FontSize(16).Bold().PaddingTop(10);
                        col.Item().Image(SvgToPng(lossChartSvg)).FitWidth();
                    }
                    if (perEpisodeStats != null && perEpisodeStats.Count > 0)
                    {
                        col.Item().Text("Per-Episode Table").FontSize(16).Bold().PaddingTop(10);
                        col.Item().Table(table =>
                        {
                            table.ColumnsDefinition(columns =>
                            {
                    ...
                    {
                        col.Item().Text("Best Episode Trajectory").FontSize(16).Bold().PaddingTop(10);
                        col.Item().Table(table =>
                        {
                            table.ColumnsDefinition(columns =>
                            {
                    ...
                    {
                        col.Item().Text("Worst Episode Trajectory").FontSize(16).Bold().PaddingTop(10);
                        col.Item().Table(table =>
                        {
                            table.ColumnsDefinition(columns =>
                            {
                ...
                page.Footer().AlignCenter().Text(x => x.Span("Generated by Gymnasium - ").FontSize(10).Italic().Span(date).FontSize(10));
            });
